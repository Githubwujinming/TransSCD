...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        return drop_path(x, self.drop_prob, self.training)# train() 会设置self.training=True
...
class CatAttention(nn.Module):
    def __init__(self, d_model, d_out):
        ...
        self.activation = nn.GELU()
        self.spatial_gating_unit = LKA(d_model)
        self.proj_2 = nn.Conv2d(d_model, d_out, 1)
...
class Block(nn.Module):
    expansion = 1
    def __init__(self, dim,ou_c=0, stride=0, downsample=0, mlp_ratio=4., drop=0.,drop_path=0.4, act_layer=nn.GELU, BatchNorm=nn.BatchNorm2d):
        super().__init__()
...
@register_model
def van18(pretrained=False, **kwargs):
    model = VAN(
        embed_dims=[18, 36, 72, 144], mlp_ratios=[8, 8, 4, 4],
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 3, 5, 2],
        **kwargs)
    return model
    
if __name__ == '__main__':
    # model = DAttention(64,dlka_type='c')
    # model = BAM(64)
    # model = LKABlock(64)
    x1 = torch.rand(1,64,64,64)
    x2 = torch.rand(1,64,64,64)
    c = torch.rand(1,64,64,64)


    # model = DBlock(64)
    # # x3 = torch.rand(1,64,64,64)
    # y1,y2, y3 = model(x1,x1,c)
    # print(y3.shape)
    # y1,y2,_ = model(x1,x2)
