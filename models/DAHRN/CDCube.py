...
class CDCube(nn.Module):
    ...
    multi_scale_output=True, norm_layer=None, siam=False, bilinear=True, fuse=True) -> None:
        super().__init__()
        #[32, 64, 160, 256]
...
class CDCubes(nn.Module):
    def __init__(self,channel_list, num_blocks=[2,2,3,3], pool_list=[True, True, True, True],cube_num=3, num_branches=4, bilinear=False, norm=nn.BatchNorm2d, siam=False, block=Block, cat=True, fuse=True) -> None:
        super().__init__()
        #[32, 64, 160, 256]
        self.cube_num = cube_num
        self.cubes = nn.ModuleList([CDCube(num_branches, block,  channel_list, pool_list, num_blocks,
                             siam=siam, bilinear=bilinear, norm_layer=norm, fuse=fuse) for i in range(cube_num)])
        self.num_branches = num_branches
        self.cat = cat
